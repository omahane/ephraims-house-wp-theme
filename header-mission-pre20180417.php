...
<body <?php body_class();?>> 
    ...
    <header>
        ...
        <?php
            $pageID = get_the_ID();
            $headlineKeys = ['headline','quote','author'];
            for ($i = 0; $i < count($headlineKeys); $i++ ) {
                $metaArray = get_post_meta($pageID, $headlineKeys[$i]);
                print('<div id="featured-image-' . $headlineKeys[$i] . '">' . $metaArray[0] . '</div>');
            }
        ?>
        </div>
